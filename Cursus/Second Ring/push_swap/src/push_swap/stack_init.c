...
static void	append_node(t_stack_n**stack, int n) //Define a function that searches for the last node to append to the linked list
{
	t_stack_n	*node; //To store a pointer to the new node to be created with the value `n`
	t_stack_n	*last_node; //To store a pointer to the current last node of the stack

	if (!stack)
		return ;
	node = malloc(sizeof(t_stack_n)); //Allocate memory for the new node
	if (!node)
		return ;
...
void	init_stack_a(t_stack_n **a, char **argv) //Define a function that initiates stack `a` by handling any errors and appending required nodes to complete a stack
{
	long	n;
	int		i;
...
t_stack_n	*get_cheapest(t_stack_n *stack) //Define a function that searches for the cheapest node, that is set by bool
{
	if (!stack)
		return (NULL);
	while (stack)
