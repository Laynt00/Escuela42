...
static void	set_target_b(t_stack_n *a, t_stack_n *b) //Define a function that sets for the current `a` node, its target node from stack `a`
{
	t_stack_n	*current_a; //To store the pointer to the current `a` node
	t_stack_n	*target_node; //To store the pointer of the target node for `b` node
	...
	while (b)
	{
		...
		if (best_match_index == LONG_MAX) //If the best match value has not changed
			b->target_n = find_min(a); //Set `b` node's target node pointer to point to the smallest number in stack `a`
		else
			b->target_n = target_node; //If the best match value has been updated, then we have a target node for the current `b` node
		b = b->next; //Move to the next `b` node for processing
	}
}

void	init_nodes_b(t_stack_n *a, t_stack_n *b) //Define a function that prepares the nodes for pushing `b` to `a`
{
	current_index(a);
	current_index(b);
	set_target_b(a, b);
