...
int	error_duplicate(t_stack_n *a, int n) //Define a function that checks for duplicate input numbers in stack `a`
{
...
void	free_stack(t_stack_n **stack) //Define a function to free a stack if there are errors
{
	t_stack_n	*tmp; //To store the next node in the stack before the current node is freed, because once a node is freed, you can't access its next pointer
	t_stack_n	*current;
...
void	free_errors(t_stack_n **a) //Define a function that, upon encountering a unique error, to free the stack and print an error message
{
	free_stack(a);
	ft_printf("Error\n");
	exit(1);
