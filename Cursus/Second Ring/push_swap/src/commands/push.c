...
static void	push(t_stack_n **dst, t_stack_n **src) //Define a function that pushes a top node, from one stack to another's top node
{
	t_stack_n	*push_node; //Used to store the pointer to the node to be pushed
...
void	pa(t_stack_n **a, t_stack_n **b, bool print) //Push on top of `b`, the top `a` and print the instruction
{
	push(a, b); 
	if (!print) 
		ft_printf("pa\n");
...
void	pb(t_stack_n **b, t_stack_n **a, bool print) //Push on top of `a`, the top `b` and print the instruction
{
	push(b, a);
	if (!print)
		ft_printf("pb\n");
