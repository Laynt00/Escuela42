...
static void	rotate_both(t_stack_n **a,
    t_stack_n **b,
    t_stack_n *cheapest_node) //Define a function that rotates both the top `a` and `b` nodes to the bottom of their stacks, if it's the cheapest move
{
	while (*b != cheapest_node->target_n
...
static void	rev_rotate_both(t_stack_n **a,
            t_stack_n **b,
            t_stack_n *cheapest_node) //Define a function that rotates both the bottom `a` and `b` nodes to the top of their stacks, if it's the cheapest move
{
	while (*b != cheapest_node->target_n
...
static void	move_a_to_b(t_stack_n **a, t_stack_n **b) //Define a function that prepares the cheapest nodes on top of the stacks for pushing `a` nodes to stack `b`, until there are three nodes left in `a`
{
	t_stack_n	*cheapest_node; //To store the pointer to the cheapest `a` node

	cheapest_node = get_cheapest(*a); 
	if (cheapest_node->above_median 
	&& cheapest_node->target_n->above_median) //If both the cheapest `a` node and its target `b` node are above the median
	rotate_both(a, b, cheapest_node);
	else if (!(cheapest_node->above_median) 
	&& !(cheapest_node->target_n->above_median)) //If both the cheapest `a` node and its target `b` node are below the median
	...
	prep_for_push(b, cheapest_node->target_n, 'b'); //Ensure the target node is at the top, ready for pushing
	pb(b, a, false);
}

static void	move_b_to_a(t_stack_n **a, t_stack_n **b) //Define a function that prepares `b`'s target `a` nodes for pushing all `b` nodes back to stack `a` 
{
	prep_for_push(a, (*b)->target_n, 'a'); //Ensure `b`'s target `a` node is on top of the stack
	pa(a, b, false); 
}

static void	min_on_top(t_stack_n **a) //Define a function that moves the smallest number to the top
{
...
void	sort_stacks(t_stack_n **a, t_stack_n **b) //Define a function that sorts stack `a` if it has more than 3 nodes
{
